...
typedef struct {
    ...
    unsigned long long
        datasize_in_buffer; /*the size of the message remained in buffer;
                               assumed to be multiple of 8bits except for the
                               last partial block at the end of the message*/
    unsigned char H[128];   /*the hash value H; 128 bytes;*/
    unsigned char A[256];   /*the temporary round value; 256 4-bit elements*/
    unsigned char
        roundconstant[64]; /*round constant for one round; 64 4-bit elements*/
...
static const unsigned char roundconstant_zero[64] = {
    ...
    0x5, 0x1, 0x2, 0x7, 0x7, 0x5, 0x0, 0x9, 0x9, 0xd, 0xa, 0x2, 0xf,
    0x5, 0x9, 0x0, 0xb, 0x0, 0x6, 0x6, 0x7, 0x3, 0x2, 0x2, 0xa};

/*The two Sboxes S0 and S1*/
static unsigned char jh_S[2][16] = {
...
static void R8(jh_hashState* state)
{
    ...
    unsigned char roundconstant_expanded[256]; /*the round constant expanded
                                                  into 256 1-bit element;*/

    /*expand the round constant into 256 one-bit element*/
    for (i = 0; i < 256; i++) {
        roundconstant_expanded[i] =
            (state->roundconstant[i >> 2] >> (3 - (i & 3))) & 1;
    }

    /*S box layer, each constant bit selects one Sbox from S0 and S1*/
    for (i = 0; i < 256; i++) {
        tem[i] = jh_S[roundconstant_expanded[i]]
                     [state->A[i]]; /*constant bits are used to determine which
                                       Sbox to use*/
    }
...
static void update_roundconstant(jh_hashState* state)
{
    ...
    /*Sbox layer*/
    for (i = 0; i < 64; i++)
        tem[i] = jh_S[0][state->roundconstant[i]];
...
/*initial group at the begining of E_8: group the bits of H into 4-bit elements
...
static void E8(jh_hashState* state)
{
    ...
    /*initial group at the begining of E_8: group the H value into 4-bit
     * elements and store them in A */
    ...
    /*de-group at the end of E_8:  decompose the 4-bit elements of A into the
     * 1024-bit H*/
                        ...
                        jh_DataLength databitlen)
{
     ...
     * first*/
    /*we assume that the size of the data in the buffer is the multiple of 8
     * bits if it is not at the end of a message*/
    ...
    /*There is data in the buffer, and the incoming data is sufficient for a
     * full block*/
    if ((state->datasize_in_buffer > 0) &&
        ((state->datasize_in_buffer + databitlen) >= 512)) {
        memcpy(state->buffer + (state->datasize_in_buffer >> 3), data,
...
jh_HashReturn jh_Final(jh_hashState* state, jh_BitSequence* hashval)
{
    ...
    if ((state->databitlen & 0x1ff) == 0) {
        /*pad the message when databitlen is multiple of 512 bits, then process
         * the padded block*/
        for (i = 0; i < 64; i++)
            state->buffer[i] = 0;
        state->buffer[0]  = 0x80;
        ...
        /*pad and process the partial block when databitlen is not multiple of
         * 512 bits, then hash the padded blocks*/
...
{
    jh_Update(state, data, len * 8);
}

static void jh_final_wrap(unsigned char* hashval, jh_hashState* state)
{
    jh_Final(state, hashval);
}

#define JH_224_DIGEST_LENGTH 28
#define JH_256_DIGEST_LENGTH 32
#define JH_384_DIGEST_LENGTH 48
#define JH_512_DIGEST_LENGTH 64
