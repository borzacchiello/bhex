 ...
 */
static u32_t ADDTO4_temp, ADDTO4_temp2;
 ...
 */
static u32_t ROTR_temp1[2], ROTR_temp2[2];
 ...
 */
static u32_t SIGMA0_temp1[2], SIGMA0_temp2[2], SIGMA0_temp3[2], SIGMA0_temp4[2];
 ...
 */
static u32_t SIGMA1_temp1[2], SIGMA1_temp2[2], SIGMA1_temp3[2], SIGMA1_temp4[2];
 ...
 *  Ch(x,y,z)   ((x & y) ^ (~x & z))
 */
static u32_t Ch_temp1[2], Ch_temp2[2], Ch_temp3[2];
 ...
 */
static u32_t Maj_temp1[2], Maj_temp2[2], Maj_temp3[2], Maj_temp4[2];
 ...
 */
static u32_t addTemp[4] = {0, 0, 0, 0};
...
static int  SHA384_512Reset(SHA512Context* context,
                            u32_t          H0[SHA512HashSize / 4]);
static void SHA384_512ProcessMessageBlock(SHA512Context* context);
static void SHA384_512Finalize(SHA512Context* context, u8_t Pad_Byte);
static void SHA384_512PadMessage(SHA512Context* context, u8_t Pad_Byte);
static int  SHA384_512ResultN(SHA512Context* context, u8_t Message_Digest[],
                              int HashSize);

/* Initial Hash Values: FIPS 180-3 sections 5.3.4 and 5.3.5 */
static u32_t SHA384_H0[SHA512HashSize / 4] = {
...
static u32_t SHA512_H0[SHA512HashSize / 4] = {
...
#define SHA512_SHR(bits, word) (((u64_t)(word)) >> (bits))
#define SHA512_ROTR(bits, word)                                                \
    ((((u64_t)(word)) >> (bits)) | (((u64_t)(word)) << (64 - (bits))))
...
/* Initial Hash Values: FIPS 180-3 sections 5.3.4 and 5.3.5 */
static u64_t SHA384_H0[] = {0xCBBB9D5DC1059ED8ll, 0x629A292A367CD507ll,
                            0x9159015A3070DD17ll, 0x152FECD8F70E5939ll,
                            0x67332667FFC00B31ll, 0x8EB44A8768581511ll,
                            0xDB0C2E0D64F98FA7ll, 0x47B5481DBEFA4FA4ll};
static u64_t SHA512_H0[] = {0x6A09E667F3BCC908ll, 0xBB67AE8584CAA73Bll,
 ...
 *
 */
int SHA384Input(SHA384Context* context, const u8_t* message_array,
                unsigned int length)
{
 ...
 *
 */
int SHA384FinalBits(SHA384Context* context, u8_t message_bits,
                    unsigned int length)
{
 ...
 *
 */
int SHA512Input(SHA512Context* context, const u8_t* message_array,
                unsigned int length)
{
    if (!context)
        return shaNull;
 ...
 *
 */
int SHA512FinalBits(SHA512Context* context, u8_t message_bits,
                    unsigned int length)
{
    static u8_t masks[8] = {
    ...
    SHA384_512Finalize(
        context, (u8_t)((message_bits & masks[length]) | markbit[length]));
 ...
 *
 */
int SHA512Result(u8_t Message_Digest[SHA512HashSize], SHA512Context* context)
{
 ...
 */
#ifdef USE_32BIT_ONLY
static int SHA384_512Reset(SHA512Context* context, u32_t H0[SHA512HashSize / 4])
#else  /* !USE_32BIT_ONLY */
static int SHA384_512Reset(SHA512Context* context, u64_t H0[SHA512HashSize / 8])
#endif /* USE_32BIT_ONLY */
{
    int i;
    if (!context)
...
static void SHA384_512ProcessMessageBlock(SHA512Context* context)
{
    ...
    u32_t temp1[2], temp2[2], /* Temporary word values */
        temp3[2], temp4[2], temp5[2];
    u32_t W[2 * 80];              /* Word sequence */
    u32_t A[2], B[2], C[2], D[2], /* Word buffers */
        E[2], F[2], G[2], H[2];

    /* Initialize the first 16 words in the array W */
    for (t = t2 = t8 = 0; t < 16; t++, t8 += 8) {
        W[t2++] = ((((u32_t)context->Message_Block[t8])) << 24) |
                  ((((u32_t)context->Message_Block[t8 + 1])) << 16) |
                  ((((u32_t)context->Message_Block[t8 + 2])) << 8) |
                  ((((u32_t)context->Message_Block[t8 + 3])));
        W[t2++] = ((((u32_t)context->Message_Block[t8 + 4])) << 24) |
                  ((((u32_t)context->Message_Block[t8 + 5])) << 16) |
                  ((((u32_t)context->Message_Block[t8 + 6])) << 8) |
                  ((((u32_t)context->Message_Block[t8 + 7])));
    }
    ...
    for (t = 16; t < 80; t++, t2 += 2) {
        ...
        u32_t* Wt2  = &W[t2 - 2 * 2];
        u32_t* Wt7  = &W[t2 - 7 * 2];
        u32_t* Wt15 = &W[t2 - 15 * 2];
        u32_t* Wt16 = &W[t2 - 16 * 2];
    ...
    u64_t temp1, temp2;           /* Temporary word value */
    u64_t W[80];                  /* Word sequence */
    u64_t A, B, C, D, E, F, G, H; /* Word buffers */

    /*
     * Initialize the first 16 words in the array W
     */
    for (t = t8 = 0; t < 16; t++, t8 += 8)
        W[t] = ((u64_t)(context->Message_Block[t8]) << 56) |
               ((u64_t)(context->Message_Block[t8 + 1]) << 48) |
               ((u64_t)(context->Message_Block[t8 + 2]) << 40) |
               ((u64_t)(context->Message_Block[t8 + 3]) << 32) |
               ((u64_t)(context->Message_Block[t8 + 4]) << 24) |
               ((u64_t)(context->Message_Block[t8 + 5]) << 16) |
               ((u64_t)(context->Message_Block[t8 + 6]) << 8) |
               ((u64_t)(context->Message_Block[t8 + 7]));
...
static void SHA384_512PadMessage(SHA512Context* context, u8_t Pad_Byte)
{
     ...
     */
#ifdef USE_32BIT_ONLY
    context->Message_Block[112] = (u8_t)(context->Length[0] >> 24);
    context->Message_Block[113] = (u8_t)(context->Length[0] >> 16);
    context->Message_Block[114] = (u8_t)(context->Length[0] >> 8);
    context->Message_Block[115] = (u8_t)(context->Length[0]);
    context->Message_Block[116] = (u8_t)(context->Length[1] >> 24);
    context->Message_Block[117] = (u8_t)(context->Length[1] >> 16);
    context->Message_Block[118] = (u8_t)(context->Length[1] >> 8);
    context->Message_Block[119] = (u8_t)(context->Length[1]);

    context->Message_Block[120] = (u8_t)(context->Length[2] >> 24);
    context->Message_Block[121] = (u8_t)(context->Length[2] >> 16);
    context->Message_Block[122] = (u8_t)(context->Length[2] >> 8);
    context->Message_Block[123] = (u8_t)(context->Length[2]);
    context->Message_Block[124] = (u8_t)(context->Length[3] >> 24);
    context->Message_Block[125] = (u8_t)(context->Length[3] >> 16);
    context->Message_Block[126] = (u8_t)(context->Length[3] >> 8);
    context->Message_Block[127] = (u8_t)(context->Length[3]);
#else  /* !USE_32BIT_ONLY */
    context->Message_Block[112] = (u8_t)(context->Length_High >> 56);
    context->Message_Block[113] = (u8_t)(context->Length_High >> 48);
    context->Message_Block[114] = (u8_t)(context->Length_High >> 40);
    context->Message_Block[115] = (u8_t)(context->Length_High >> 32);
    context->Message_Block[116] = (u8_t)(context->Length_High >> 24);
    context->Message_Block[117] = (u8_t)(context->Length_High >> 16);
    context->Message_Block[118] = (u8_t)(context->Length_High >> 8);
    context->Message_Block[119] = (u8_t)(context->Length_High);

    context->Message_Block[120] = (u8_t)(context->Length_Low >> 56);
    context->Message_Block[121] = (u8_t)(context->Length_Low >> 48);
    context->Message_Block[122] = (u8_t)(context->Length_Low >> 40);
    context->Message_Block[123] = (u8_t)(context->Length_Low >> 32);
    context->Message_Block[124] = (u8_t)(context->Length_Low >> 24);
    context->Message_Block[125] = (u8_t)(context->Length_Low >> 16);
    context->Message_Block[126] = (u8_t)(context->Length_Low >> 8);
    context->Message_Block[127] = (u8_t)(context->Length_Low);
#endif /* USE_32BIT_ONLY */
                             ...
                             int HashSize)
{
    ...
    for (i = i2 = 0; i < HashSize;) {
        Message_Digest[i++] = (u8_t)(context->Intermediate_Hash[i2] >> 24);
        Message_Digest[i++] = (u8_t)(context->Intermediate_Hash[i2] >> 16);
        Message_Digest[i++] = (u8_t)(context->Intermediate_Hash[i2] >> 8);
        Message_Digest[i++] = (u8_t)(context->Intermediate_Hash[i2++]);
        Message_Digest[i++] = (u8_t)(context->Intermediate_Hash[i2] >> 24);
        Message_Digest[i++] = (u8_t)(context->Intermediate_Hash[i2] >> 16);
        Message_Digest[i++] = (u8_t)(context->Intermediate_Hash[i2] >> 8);
        Message_Digest[i++] = (u8_t)(context->Intermediate_Hash[i2++]);
    ...
    for (i = 0; i < HashSize; ++i)
        Message_Digest[i] =
            (u8_t)(context->Intermediate_Hash[i >> 3] >> 8 * (7 - (i % 8)));
#endif /* USE_32BIT_ONLY */
