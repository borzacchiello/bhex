...
#define STORE32LE(a, p)                                                        \
    ((u8_t*)(p))[0] = ((u32_t)(a) >> 0) & 0xFFU,                               \
    ((u8_t*)(p))[1] = ((u32_t)(a) >> 8) & 0xFFU,                               \
    ((u8_t*)(p))[2] = ((u32_t)(a) >> 16) & 0xFFU,                              \
    ((u8_t*)(p))[3] = ((u32_t)(a) >> 24) & 0xFFU

// Load unaligned 32-bit integer (little-endian encoding)
#define LOAD32LE(p)                                                            \
    (((u32_t)(((u8_t*)(p))[0]) << 0) | ((u32_t)(((u8_t*)(p))[1]) << 8) |       \
     ((u32_t)(((u8_t*)(p))[2]) << 16) | ((u32_t)(((u8_t*)(p))[3]) << 24))
...
const u8_t RIPEMD160_OID[5] = {0x2B, 0x24, 0x03, 0x02, 0x01};
...
void ripemd160ProcessBlock(Ripemd160Context* context)
{
    ...
    u32_t aa  = context->h[0];
    u32_t bb  = context->h[1];
    u32_t cc  = context->h[2];
    u32_t dd  = context->h[3];
    u32_t ee  = context->h[4];
    u32_t aaa = context->h[0];
    u32_t bbb = context->h[1];
    u32_t ccc = context->h[2];
    u32_t ddd = context->h[3];
    u32_t eee = context->h[4];
