/*
 * Tiger implementation, based on the article by Ross Anderson and Eli Biham
 * "Tiger: A Fast New Hash Function".
 *
 * Derived from the RHash implementation by Aleksey Kravchenko. The RHash
 * version selects between an x86-64 and an IA32 flavour of the round macro
 * and reads message blocks in place when they happen to be 64-bit aligned;
 * this adaptation keeps a single portable round and always decodes the block
 * byte by byte, so no alignment or word-size assumptions are made.
 *
...
static u64_t dec64le(const u8_t* p)
{
    return (u64_t)p[0] | ((u64_t)p[1] << 8) | ((u64_t)p[2] << 16) |
           ((u64_t)p[3] << 24) | ((u64_t)p[4] << 32) | ((u64_t)p[5] << 40) |
           ((u64_t)p[6] << 48) | ((u64_t)p[7] << 56);
}

static void enc64le(u8_t* p, u64_t v)
{
    p[0] = (u8_t)v;
    p[1] = (u8_t)(v >> 8);
    p[2] = (u8_t)(v >> 16);
...
/* The four S-boxes used for table lookups, 8Kb in total. */

static const u64_t tiger_sboxes[4][256] = {
...
#define round(a, b, c, x, mul)                                                 \
    c ^= x;                                                                    \
    a -= t1[(u8_t)(c)] ^ t2[(u8_t)((c) >> (2 * 8))] ^                          \
         t3[(u8_t)((c) >> (4 * 8))] ^ t4[(u8_t)((c) >> (6 * 8))];              \
    b += t4[(u8_t)((c) >> (1 * 8))] ^ t3[(u8_t)((c) >> (3 * 8))] ^             \
         t2[(u8_t)((c) >> (5 * 8))] ^ t1[(u8_t)((c) >> (7 * 8))];              \
    b *= mul;

#define pass(a, b, c, mul)                                                     \
    round(a, b, c, x0, mul) round(b, c, a, x1, mul) round(c, a, b, x2, mul)    \
        round(a, b, c, x3, mul) round(b, c, a, x4, mul)                        \
            round(c, a, b, x5, mul) round(a, b, c, x6, mul)                    \
                round(b, c, a, x7, mul)
...
/* The core transformation: process one 512-bit block. */
static void tiger_process_block(u64_t state[3], const u8_t* block)
{
    u64_t a, b, c;
    u64_t x0, x1, x2, x3, x4, x5, x6, x7;
...
static void tiger_init_state(TigerCtx* ctx, u8_t pad)
{
    ctx->hash[0] = 0x0123456789ABCDEFULL;
    ctx->hash[1] = 0xFEDCBA9876543210ULL;
    ctx->hash[2] = 0xF096A5B4C3B2E187ULL;
    ctx->count   = 0;
    ctx->pad     = pad;
}

void tiger_init(TigerCtx* ctx) { tiger_init_state(ctx, 0x01); }

void tiger2_init(TigerCtx* ctx) { tiger_init_state(ctx, 0x80); }

void tiger_update(TigerCtx* ctx, const u8_t* data, u64_t len)
{
    u64_t index = ctx->count & (TIGER_BLOCK_SIZE - 1);
    ctx->count += len;

    /* fill the partial block left over by the previous call */
    if (index) {
        u64_t left = TIGER_BLOCK_SIZE - index;
        if (len < left) {
            if (len > 0)
                memcpy(ctx->buffer + index, data, (size_t)len);
            return;
        }
        memcpy(ctx->buffer + index, data, (size_t)left);
        tiger_process_block(ctx->hash, ctx->buffer);
        data += left;
        len -= left;
    }
    while (len >= TIGER_BLOCK_SIZE) {
        tiger_process_block(ctx->hash, data);
...
{
    unsigned index = (unsigned)(ctx->count & (TIGER_BLOCK_SIZE - 1));

    /* the padding byte is what tells the two variants apart */
    ctx->buffer[index++] = ctx->pad;

    /* if no room is left for the 64-bit message length, fill the rest of the
     * block with zeros and process it */
    if (index > 56) {
        memset(ctx->buffer + index, 0, TIGER_BLOCK_SIZE - index);
        tiger_process_block(ctx->hash, ctx->buffer);
        index = 0;
    }
    memset(ctx->buffer + index, 0, 56 - index);
    enc64le(ctx->buffer + 56, ctx->count << 3);
    tiger_process_block(ctx->hash, ctx->buffer);

    enc64le(digest, ctx->hash[0]);
    enc64le(digest + 8, ctx->hash[1]);
    enc64le(digest + 16, ctx->hash[2]);
}
