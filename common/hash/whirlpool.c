 ...
 * version reads message blocks in place when they happen to be 64-bit
 * aligned; this adaptation always decodes the block byte by byte, so no
 * alignment or word-size assumptions are made.
 *
...
static u64_t dec64be(const u8_t* p)
{
    return ((u64_t)p[0] << 56) | ((u64_t)p[1] << 48) | ((u64_t)p[2] << 40) |
           ((u64_t)p[3] << 32) | ((u64_t)p[4] << 24) | ((u64_t)p[5] << 16) |
           ((u64_t)p[6] << 8) | (u64_t)p[7];
}

static void enc64be(u8_t* p, u64_t v)
{
    p[0] = (u8_t)(v >> 56);
    p[1] = (u8_t)(v >> 48);
    p[2] = (u8_t)(v >> 40);
...
/* The eight S-box tables, one per byte position, 16Kb in total. */

static const u64_t whirlpool_sbox[8][256] = {
    {
        /* C0 vectors */
...
/* One column of the round transformation: pick one byte out of each of the
 * eight state words, following the diffusion pattern, and combine the
...
static void whirlpool_process_block(u64_t* hash, const u8_t* block)
{
    /* the round constants of the internal block cipher */
    static const u64_t rc[10] = {0x1823c6e887b8014fULL, 0x36a6d2f5796f9152ULL,
    ...
    unsigned m = 0; /* alternating binary flag, selects the current pair */
    int      i;
    ...
    for (i = 0; i < 8; i++) {
        ...
        K[0][i]     = hash[i];
        state[0][i] = dec64be(block + i * 8) ^ hash[i];
        hash[i]     = state[0][i];
    }
...
{
    u64_t index = ctx->count & (WHIRLPOOL_BLOCK_SIZE - 1);
    ctx->count += len;

    /* fill the partial block left over by the previous call */
    if (index) {
        u64_t left = WHIRLPOOL_BLOCK_SIZE - index;
        if (len < left) {
            if (len > 0)
        ...
        whirlpool_process_block(ctx->hash, ctx->buffer);
        data += left;
        len -= left;
    }
    while (len >= WHIRLPOOL_BLOCK_SIZE) {
        whirlpool_process_block(ctx->hash, data);
...
{
    unsigned index = (unsigned)(ctx->count & (WHIRLPOOL_BLOCK_SIZE - 1));
    int      i;

    ctx->buffer[index++] = 0x80;

    /* if no room is left for the 256-bit message length, fill the rest of the
    ...
    /* only the low 64 bits of the length counter are kept, the rest of the
     * 256-bit length field stays zero */
    memset(ctx->buffer + index, 0, 56 - index);
    enc64be(ctx->buffer + 56, ctx->count << 3);
    whirlpool_process_block(ctx->hash, ctx->buffer);

    for (i = 0; i < 8; i++)
        enc64be(digest + i * 8, ctx->hash[i]);
}
