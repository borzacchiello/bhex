                              ...
                              size_t out_len)
{
    ...
    if (offset_within_block) {
        ...
        const size_t bytes =
            out_len > available_bytes ? available_bytes : out_len;
                                ...
                                bool use_tbb)
{
    ...
    // (particularly GCC 8.5) can't tell that it never executes, and if NDEBUG
    // is set then it emits incorrect warnings here. We tried a few different
    ...
    while (input_len > BLAKE3_CHUNK_LEN) {
        ...
        if (subtree_len <= BLAKE3_CHUNK_LEN) {
            ...
            compress_subtree_to_parent_node(
                input_bytes, subtree_len, self->key, self->chunk.chunk_counter,
...
/* Wrapper for GEN_HANDLE_FUNC macro: swaps argument order */
void blake3_hasher_finalize_std(uint8_t* out, blake3_hasher* self)
{
    blake3_hasher_finalize(self, out, BLAKE3_OUT_LEN);
}
