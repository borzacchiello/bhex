...
static u32_t gost_sbox_1[256];
static u32_t gost_sbox_2[256];
static u32_t gost_sbox_3[256];
static u32_t gost_sbox_4[256];
...
static void gost_compress(u32_t* h, u32_t* m)
{
    int   i;
    u32_t l, r, t, key[8], u[8], v[8], w[8], s[8];
    ...
    u[3] = m[3] ^ (s[0] & 0xffff) ^ (s[0] << 16) ^ (s[1] & 0xffff) ^
           ...
           (s[7] << 16) ^ (s[7] >> 16);
    u[4] = m[4] ^ (s[0] & 0xffff0000) ^ (s[0] << 16) ^ (s[0] >> 16) ^
...
static void gost_bytes(GostHashCtx* ctx, const uint8_t* buf, u32_t bits)
{
    int   i, j;
    u32_t a, c, m[8];
    ...
    c = 0;
    for (i = 0; i < 8; i++) {
        a = ((u32_t)buf[j]) | (((u32_t)buf[j + 1]) << 8) |
            (((u32_t)buf[j + 2]) << 16) | (((u32_t)buf[j + 3]) << 24);
        j += 4;
        m[i]        = a;
        c           = a + c + ctx->sum[i];
        ctx->sum[i] = c;
...
void GHOSTFinal(uint8_t digest[GHOST_DIGEST_LENGTH], GostHashCtx* ctx)
{
    ...
    if (ctx->partial_bytes > 0) {
        ...
        gost_bytes(ctx, ctx->partial, ctx->partial_bytes << 3);
    }
