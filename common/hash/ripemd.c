...
static void ripemd128_transform(struct ripemd_ctx* ctx, u32_t* data)
{
    u32_t A, B, C, D;     /* Local vars */
    u32_t AA, BB, CC, DD; /* Local vars */
    u32_t T;
...
static void ripemd160_transform(struct ripemd_ctx* ctx, u32_t* data)
{
    u32_t A, B, C, D, E;      /* Local vars */
    u32_t AA, BB, CC, DD, EE; /* Local vars */
    u32_t T;
...
static void ripemd256_transform(struct ripemd_ctx* ctx, u32_t* data)
{
    u32_t A, B, C, D;     /* Local vars */
    u32_t AA, BB, CC, DD; /* Local vars */
    u32_t T;
...
static void ripemd320_transform(struct ripemd_ctx* ctx, u32_t* data)
{
    u32_t A, B, C, D, E;      /* Local vars */
    u32_t AA, BB, CC, DD, EE; /* Local vars */
    u32_t T;
...
static void ripemd_transform(struct ripemd_ctx* ctx, u32_t* data)
{
    switch (ctx->digest_len) /* select the right compression function */
    {
        case 8 * RIPEMD128_DIGESTSIZE:
            ripemd128_transform(ctx, data);
            break;
        case 8 * RIPEMD160_DIGESTSIZE:
            ripemd160_transform(ctx, data);
            break;
        case 8 * RIPEMD256_DIGESTSIZE:
            ripemd256_transform(ctx, data);
            break;
        case 8 * RIPEMD320_DIGESTSIZE:
            ripemd320_transform(ctx, data);
            break;
    }
}

#ifndef EXTRACT_UCHAR
#define EXTRACT_UCHAR(p) (*(u8_t*)(p))
#endif

#define STRING2INT(s)                                                          \
    (((((((EXTRACT_UCHAR(s + 3) << 8) | EXTRACT_UCHAR(s + 2)) << 8) |          \
        EXTRACT_UCHAR(s + 1)) &                                                \
       0xffffffu)                                                              \
      << 8) |                                                                  \
     EXTRACT_UCHAR(s))

static void ripemd_block(struct ripemd_ctx* ctx, const u8_t* block)
{
    u32_t data[RIPEMD_DATALEN];
    u32_t i;
...
void ripemd_final(u8_t* s, struct ripemd_ctx* ctx)
{
    ...
    if (words >
        (RIPEMD_DATALEN - 2)) { /* No room for length in this block.
    ...
    data[RIPEMD_DATALEN - 2] = ctx->bitcount & ((1LL << 32) - 1);
    data[RIPEMD_DATALEN - 1] = ctx->bitcount >> 32;

    ripemd_transform(ctx, data);

    if (s != NULL)
        for (u32_t i = 0; i < ctx->digest_len / 32; i++) {
            *s++ = 0xff & ctx->digest[i];
