...
{ /* do it in C */
    ...
    do {
        ...
        Skein_Get64_LSB_First(
            w, blkPtr, WCNT); /* get input block in little-endian format */
        ...
        Skein_Show_Round(BLK_BITS, &ctx->h, SKEIN_RND_KEY_INITIAL,
                         X); /* show starting state values */

        for (r = 1; r <= SKEIN_256_ROUNDS_TOTAL / 8;
             r++) { /* unroll 8 rounds */
            X[0] += X[1];
...
uint_t Skein_256_Unroll_Cnt(void) { return 1; }
#endif
...
{ /* do it in C */
    ...
    do {
        ...
        for (r = 1; r <= SKEIN_512_ROUNDS_TOTAL / 8;
             r++) { /* unroll 8 rounds */
            X[0] += X[1];
...
uint_t Skein_512_Unroll_Cnt(void) { return 1; }
#endif
...
{ /* do it in C */
    ...
    do {
        ...
        Skein_Show_Round(BLK_BITS, &ctx->h, SKEIN_RND_KEY_INITIAL,
                         X); /* show starting state values */
        for (r = 1; r <= SKEIN1024_ROUNDS_TOTAL / 8;
             r++) { /* unroll 8 rounds */
            X[0] += X[1];
...
uint_t Skein1024_Unroll_Cnt(void) { return 1; }
#endif
