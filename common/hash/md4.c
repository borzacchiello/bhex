 ...
 * u32_t.  In practice, this problem may occur if these MD4 routines are
 * inlined into a calling function, or with future and dangerously advanced
 * link-time optimizations.  For the time being, keeping these MD4 routines in
 * their own translation unit avoids the problem.
 */
#if defined(__i386__) || defined(__x86_64__) || defined(__vax__)
#define SET(n) (*(u32_t*)&ptr[(n) * 4])
#define GET(n) SET(n)
#else
#define SET(n)                                                                 \
    (ctx->block[(n)] = (u32_t)ptr[(n) * 4] | ((u32_t)ptr[(n) * 4 + 1] << 8) |  \
                       ((u32_t)ptr[(n) * 4 + 2] << 16) |                       \
                       ((u32_t)ptr[(n) * 4 + 3] << 24))
...
static const void* body(MD4_CTX* ctx, const void* data, unsigned long size)
{
    ...
    u32_t                a, b, c, d;
    u32_t                saved_a, saved_b, saved_c, saved_d;
    const u32_t          ac1 = 0x5a827999, ac2 = 0x6ed9eba1;
...
void MD4Final(u8_t result[MD4_DIGEST_LENGTH], MD4_CTX* ctx)
{
    unsigned long used, available;
