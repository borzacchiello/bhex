...
bhex [ options ] inputfile
  ...
  -U  --no_unicode  draw with ASCII only
...
| hash | `hh` | `hh[/l] <algo> [<size> <off>]` | `off` relative to the cursor; `size` 0/omitted = to EOF; `*` = all; `/l` lists; a name is read at the narrowest tier that matches: exact first (`hh blake2b` is only `blake2b`, not `blake2b-256`), then prefix (`hh md` is the MD family, and does *not* include RipeMD), then anywhere (`hh 256` catches every name containing it). 86 algorithms: md2/4/5/6, sm3, sha1, sha2 (incl. `sha512-224`/`sha512-256`), sha3, `shake128-256`/`shake256-512`, `keccak-*` (pre-FIPS-202 padding, what Ethereum calls SHA-3 — not the same as `sha3-*`), ripemd, blake2s/2b (several digest sizes), blake3, `gost` (34.11-94) and `streebog-*` (34.11-2012), groestl, jh, skein, snefru, spectral, haval, tiger, whirlpool, and the non-cryptographic `xxh32`/`xxh64`/`xxh3-64`/`xxh3-128`, `murmur3-32`/`murmur3-128`, `fnv1-*`/`fnv1a-*` |
...
| disas | `ds` | `ds[/l\|/o] <arch> [<n>]` | Capstone; `l` lists the architectures; `o` prints the bytes of every instruction between the address and the mnemonic (off by default); `n` = instruction count, unbounded (the listing is read and printed one 4096 bytes block at a time); with no `n` the listing is the function at the cursor, ending at the first instruction that returns (per architecture: `ret`, `blr`, `jr $ra` + its delay slot, a write of the return address into the program counter, ...), or at the end of the file or the first bytes that decode to nothing (printed as `invalid`); an operand counted from the program counter (x86 `[rip + N]`, arm literal pools, riscv `auipc`) is followed by the address it resolves to as `; 0x...`, `setbase` included; control flow instructions are highlighted |
| assemble | `as` | `as[/l/i/s] <arch> '<code>'` | Keystone; writes at the cursor, `i` insert, `s` seek to the end |
| identify | `id` | `id[/l/v/vv/n/e] [<len>]` | scans for the formats the templates declare a magic for; a hit reports the size, and the scan resumes past it (`n` does not skip), `l` lists the templates taking part, `e` ignores the magics and asks every template at every offset, `v`/`vv` add the numbers of the scan; `len` counts from the cursor |
| isa_identify | `ii` | `ii[/s] [<size>]` | bundled models; by default scans in 1024-byte chunks and prints the detected code ranges with the top ISA of each; `/s` instead names the top ISA candidates for the whole range, for a range already known to be code; the confidence is colored by band |
